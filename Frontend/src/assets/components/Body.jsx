...
const Body = () => {
  ...
  const topCompanies = [
    ...
    "Dominos",
    "BookMyShow",
    "CodingGita Bootcamp" // नवीन कंपनी ॲड केली
  ];

  // API मधून कूपन्स फेच करणे
  useEffect(() => {
    const fetchCoupons = async () => {
      try {
        setLoading(true);
        const response = await axios.get('https://coupon-swap-backend.onrender.com/api/coupon', {
          timeout: 10000 // 10 सेकंदांचा टाइमआउट
        });

        // रिस्पॉन्स डेटा तपासणे
        let coupons = response.data;
        
        // जर रिस्पॉन्स ऑब्जेक्ट असेल आणि त्यात 'data' फील्ड असेल, तर ती ॲरे वापर
        if (!Array.isArray(coupons) && coupons.data && Array.isArray(coupons.data)) {
          coupons = coupons.data;
        }

        // रिस्पॉन्स ॲरे आहे का, तपासणे
        if (!Array.isArray(coupons)) {
          throw new Error(`API response is not an array: ${JSON.stringify(coupons)}`);
        }

        // कूपन्स मॅप करणे आणि आवश्यक फील्ड्स व्हॅलिडेट करणे
        const validCoupons = coupons
          .map(coupon => ({
            id: coupon._id || coupon.id,
            platform: coupon.platform,
            value: coupon.valueRs || coupon.value,
            price: coupon.sellingPrice || coupon.price,
            expiryDate: coupon.expiryDate,
            seller: coupon.sellerName || coupon.seller,
            rating: coupon.rating || 4.0, // डिफॉल्ट रेटिंग
            category: coupon.category || "Other", // डिफॉल्ट कॅटेगरी
            discount: coupon.discount || Math.round(((coupon.valueRs - coupon.sellingPrice) / coupon.valueRs) * 100) || 20, // डिफॉल्ट डिस्काउंट
            code: coupon.couponCode || coupon.code
          }))
          .filter(coupon => {
            if (!coupon.id || !coupon.platform || !coupon.value || !coupon.price || !coupon.expiryDate || !coupon.seller || !coupon.rating || !coupon.category || !coupon.discount || !coupon.code) {
              console.warn(`Invalid coupon data: ${JSON.stringify(coupon)}`);
              return false;
            }
            return true;
          });

        // एक्सपायरी डेट तपासून वैध कूपन्स फिल्टर करणे
        ...
        const filteredCoupons = validCoupons
          .filter(coupon => {
            try {
              ...
              console.warn(`Invalid expiryDate for coupon ID ${coupon.id}: ${coupon.expiryDate}`);
              return false;
            }
          })
          // टॉप कंपन्यांचे कूपन्स प्राधान्याने दाखवणे
          .sort((a, b) => {
            const aIsTop = topCompanies.includes(a.platform) ? -1 : 1;
            const bIsTop = topCompanies.includes(b.platform) ? -1 : 1;
        ...
        setError('Failed to fetch coupons from API. Showing default coupons.');
        setLoading(false);

        // डिफॉल्ट स्टॅटिक डेटा
        const currentDate = new Date();
        const defaultCoupons = [
          { id: 1, platform: "Google Pay", value: 100, price: 80, expiryDate: "2025-09-15", seller: "Rahul S.", rating: 4.8, category: "Payment", discount: 20, code: "GPAY100" },
          { id: 2, platform: "Paytm", value: 50, price: 40, expiryDate: "2025-09-20", seller: "Priya M.", rating: 4.9, category: "Payment", discount: 20, code: "PAYTM50" },
          { id: 3, platform: "Amazon", value: 250, price: 200, expiryDate: "2025-09-17", seller: "Anjali T.", rating: 4.8, category: "E-commerce", discount: 20, code: "AMZ250" },
          { id: 4, platform: "Flipkart", value: 500, price: 400, expiryDate: "2025-09-28", seller: "Kiran D.", rating: 4.7, category: "E-commerce", discount: 20, code: "FK500" },
          { id: 5, platform: "Myntra", value: 300, price: 240, expiryDate: "2025-09-23", seller: "Ravi B.", rating: 4.6, category: "Fashion", discount: 20, code: "MYN300" },
          { id: 6, platform: "Zomato", value: 200, price: 160, expiryDate: "2025-09-25", seller: "Sneha K.", rating: 4.5, category: "Food", discount: 20, code: "ZOM200" },
          { id: 7, platform: "Nike", value: 1000, price: 800, expiryDate: "2025-10-01", seller: "Vikram P.", rating: 4.9, category: "Fashion", discount: 20, code: "NIKE1000" },
          { id: 8, platform: "Dominos", value: 150, price: 120, expiryDate: "2025-09-30", seller: "Amit J.", rating: 4.7, category: "Food", discount: 20, code: "DOM150" },
          { id: 9, platform: "CodingGita Bootcamp", value: 600, price: 10, expiryDate: "2025-12-30", seller: "T", rating: 4.0, category: "Education", discount: 98, code: "CGH105" }
        ].filter(coupon => new Date(coupon.expiryDate) >= currentDate);

        setValidCoupons(defaultCoupons);
      }
    };
  ...
  // कॅटेगरी काउंट्स
  const categories = [
    { name: "All", count: validCoupons.length },
    { name: "Payment", count: validCoupons.filter(c => c.category === "Payment").length },
    { name: "E-commerce", count: validCoupons.filter(c => c.category === "E-commerce").length },
    { name: "Fashion", count: validCoupons.filter(c => c.category === "Fashion").length },
    { name: "Food", count: validCoupons.filter(c => c.category === "Food").length },
    { name: "Education", count: validCoupons.filter(c => c.category === "Education").length } // नवीन कॅटेगरी
  ];
    ...
    <main className="flex-grow">
        ...
        <div className="container mx-auto max-w-7xl">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div className="space-y-6">
              ...
              <p className="text-lg text-gray-700">
                Trade your Amazon, Flipkart, or other coupons for cash or snag incredible deals.
              </p>
              <div className="flex flex-col sm:flex-row gap-4">
                <button className="px-6 py-3 bg-gradient-to-r from-orange-600 to-amber-500 text-white font-medium rounded-full hover:from-orange-700 hover:to-amber-600 transition-all shadow-lg flex items-center justify-center gap-2">
                  <ShoppingBag size={20} />
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {loading && <div className="text-center text-gray-600">Loading coupons...</div>}
          {error && <div className="text-center text-red-600">{error}</div>}

          {!loading && !error && filteredCoupons.length === 0 && (
            <div className="text-center text-gray-600">No coupons available for the selected filters.</div>
          )}
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center">
                      ...
                      <div>
                        <h3 className="font-semibold text-lg text-gray-900">{coupon.platform}</h3>
                        <p className="text-sm text-gray-500 flex items-center gap-1">
                          <Clock size={14} /> Expires: {new Date(coupon.expiryDate).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                    <button onClick={() => toggleFavorite(coupon.id)} className="p-1 hover:text-red-500">
